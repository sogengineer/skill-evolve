...
1. リフレクタが `structural` と診断した theme は **コーチが触らない**。
   `structural_themes_for_promotion.yaml` に分けて記録(Step 9 で permanent buffer へ即時昇格)
...
### 修正の波及パターン(事前見積もりの規律)
- **保守的に振れる**: 1 修正で複数軸狙ったが 1 軸しか動かない
- **上振れ**: 構造的情報(コマンド + 設定 + 期待出力 等)が複数軸に効く
- **ゼロ振れ**: 軸名から推測しただけで判定文言に届かない(一番ありがち)
- 各編集案で `waves_pattern` を自己診断する。上振れを申告する場合は
  必ず「構造的情報の組合せ」になっていること
