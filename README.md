...
- [背景:乗っている方法論(前提知識なしで読める版)](#背景乗っている方法論前提知識なしで読める版)
- [概要](#概要)
- [なぜ必要か(背景)](#なぜ必要か背景)
- [処理フロー](#処理フロー)
- [詳細説明](#詳細説明)
...
- 頻繁に使われる対象スキルを堅牢化したい
- 自動改善ループの収束が悪い、または偽陽性 / 偽陰性が増えてきた
- 新規スキルを追加した直後、現実の入力で精度を確認したい
...
| 編集生成 | 人間が考える | コーチが 3 操作で提案 |
| 試走の構造 | シナリオ並列 | 並列ミニバッチ + ミニバッチ単位リフレクション + 提案統合 |
| 検証 | 同シナリオ + 収束時のみ hold-out | 毎イテレーション held-out validation |
| 失敗の長期記憶 | なし | per-run + permanent の 2 層 |
...
人間の質的判断(構造欠陥のシグナル読み、修正の波及パターン認識)は機械的には埋まらないため、両者は補完関係にある。
...
本 SKILL で `rejection-buffer-permanent.yaml` と `coach_meta_prompt_v*.md` を蓄積しておくと、v2 移行時の入力として活用できる。
...
skill-evolve/
├── README.md                  ← このファイル
├── SKILL.md                   ← 汎用 skill-evolve 本体(.claude/skills/ にコピーして使う)
└── examples/
...
2. `examples/pilot-template.md` を雛形に Step -1 を実施(`feedback_spec.md` / `coach_meta_prompt_v0.md` / `pilot_report.md` を作成)
3. test_cases を準備(最低 8 ケース、`[critical]` を含む rubric 付き)
4. Claude Code から本スキルを起動し、入力契約 YAML を渡す
